...
func (c *config) loadConfigFromArgs(log *zap.Logger, cmd *cobra.Command) {
	...
	if fenixflixAPIURL != "" {
		os.Setenv("FENIXFLIX_API_URL", fenixflixAPIURL)
	}
	fenixflixPassword, _ := cmd.Flags().GetString("fenixflix-password")
	if fenixflixPassword != "" {
		os.Setenv("FENIXFLIX_PASSWORD", fenixflixPassword)
	}
